    ...
    public class SmartApi
    {
        ...
        public OutputBaseClass GenerateSession(string clientcode, string password, string totp)
        {
            ...
            try
            {
                ...
                string PostData = "{\"clientcode\":\"" + clientcode + "\",\"password\":\"" + password + "\",\"totp\":\"" + int.Parse(totp) + "\"}";
        ...
        public AOResponse GetQuoteData(QuoteData req)
        {
            ...
            try
            {
                ...
                if (Token != null)
                {
                    if (ValidateToken(Token))
                    {
                        string URL = APIURL + "/rest/secure/angelbroking/market/v1/quote/";
                        ...
                        if (!Json.Contains("PostError:"))
                        {
                            res= JsonConvert.DeserializeObject<AOResponse>(Json);
                          //  res.GetLTPDataResponse = ltp;
